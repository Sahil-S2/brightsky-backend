    ...
    const queries = [
      db.query(
        `SELECT w.* FROM worksites w
         ...
         WHERE ew.employee_id = $1 AND ew.is_default = true LIMIT 1`,
        [req.user!.id]
      ),
      db.query(
        `SELECT w.* FROM worksites w
      ...
      db.query("SELECT * FROM worksites LIMIT 1"),
    ];
...
router.post("/", verifyJWT, requireRole("admin","manager"), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO worksites (name, project_name, address, latitude, longitude, radius_feet, notes, created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [name, projectName||name, address||null, latitude, longitude, radiusFeet||200, notes||null, req.user!.id]
    );
...
router.put("/:id", verifyJWT, requireRole("admin","manager"), async (req: AuthRequest, res: Response) => {
  try {
    const { name, projectName, address, latitude, longitude, radiusFeet, notes } = req.body;
    const { rows } = await db.query(
      `UPDATE worksites SET
        name=$1, project_name=$2, address=$3,
        latitude=$4, longitude=$5, radius_feet=$6,
        notes=$7, updated_at=NOW()
       WHERE id=$8 RETURNING *`,
      [name, projectName||name, address||null, latitude, longitude, radiusFeet||200, notes||null, req.params.id]
    );
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: "Server error" });
...
router.delete("/:id", verifyJWT, requireRole("admin","manager"), async (req: AuthRequest, res: Response) => {
  try {
    await db.query("DELETE FROM employee_worksites WHERE worksite_id=$1", [req.params.id]);
...
router.get("/:id/unassigned", verifyJWT, requireRole("admin","manager"), async (req: AuthRequest, res: Response) => {
  try {
    const { rows } = await db.query(
      `SELECT u.id, COALESCE(u.full_name,u.name) as name, u.user_id,
